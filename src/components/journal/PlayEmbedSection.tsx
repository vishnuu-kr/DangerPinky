  ...
  return (
    <div className="my-1">
      ...
      >
        ...
        <div
          className={`relative w-full rounded-xl overflow-hidden border border-slate-800 bg-black flex-1 ${
            isFullscreen ? 'h-[calc(100vh-65px)]' : 'h-[310px] sm:h-[330px]'
          }`}
        >
          <iframe
            key={reloadKey}
          ...
          <div className="mt-4 pt-3 border-t border-slate-800/80 flex flex-wrap items-center justify-between gap-3 text-xs font-mono text-slate-400">
            <div className="flex flex-wrap items-center gap-4">
              <span className="flex items-center gap-1.5 text-pink-300 font-semibold">
                ...
                <span>Pinky: Landmark 20 Tip Flick</span>
              </span>
              <span className="flex items-center gap-1.5 text-emerald-300 font-semibold">
                <Keyboard className="w-4 h-4 text-emerald-400" />
                <span>Keys: Arrow Keys / WASD</span>
              </span>
              <span className="flex items-center gap-1 text-slate-300">
                <kbd className="px-1.5 py-0.5 rounded bg-slate-800 text-slate-200 border border-slate-700 text-[10px]">
                  Space
                </kbd>
                <span>to Pause</span>
              </span>
            ...
            <div className="flex items-center gap-2">
              {onStartDemo && (
                <button
                  onClick={onStartDemo}
                  className="px-3 py-1 rounded-full bg-pink-500/15 hover:bg-pink-500/25 border border-pink-500/30 text-pink-300 text-xs font-mono font-bold flex items-center gap-1 cursor-pointer transition-colors"
                >
                  <Play className="w-3 h-3 fill-pink-400 text-pink-400" />
                  <span>Launch In-App Demo</span>
                </button>
              )}
              ...
              {onStartWithCamera && (
                <button
                  ...
                  className="px-3 py-1 rounded-full bg-cyan-500/15 hover:bg-cyan-500/25 border border-cyan-500/30 text-cyan-300 text-xs font-mono font-bold flex items-center gap-1 cursor-pointer transition-colors"
                >
              ...
              {onSelectRealFiles && (
                <button
                  ...
                  className="px-3 py-1 rounded-full bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/30 text-amber-300 text-xs font-mono font-bold flex items-center gap-1 cursor-pointer transition-colors"
                >
      ...
      <div className="text-center mt-3 text-xs font-mono text-slate-500">
        ...
        Webcam permission is requested only when activating pinky tracking. Keyboard controls work immediately.
      </div>
    </div>
  );
};
